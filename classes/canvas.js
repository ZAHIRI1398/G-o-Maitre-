...
export class CanvasPage {
  ...
  redraw() {
    ...
    // Dessiner le PDF en arrière-plan si disponible
    if (this.pdfCanvas && this.pdfCanvas.width > 0 && this.pdfCanvas.height > 0) {
      try {
        console.log(`Dessin du PDF: pdfCanvas ${this.pdfCanvas.width}x${this.pdfCanvas.height} -> canvas ${this.canvas.width}x${this.canvas.height}`);
        this.ctx.drawImage(this.pdfCanvas, 0, 0);
      } catch (error) {
        console.error('Erreur lors du dessin du PDF:', error);
      }
    } else {
      console.log('Pas de PDF à dessiner ou dimensions invalides');
    }
    
    // Dessiner tous les éléments géométriques
    this.drawings.forEach(drawing => {
      try {
        drawing.draw();
      } catch (error) {
        console.error('Erreur lors du dessin d\'un élément:', error);
      }
    });
    ...
    if (this.textTool) {
      try {
        ...
        console.error('Erreur lors du dessin des textes:', error);
      }
    }
  }
  ...
  handleClick(event) {
    ...
    switch (this.activeTool) {
        ...
        for (let i = this.drawings.length - 1; i >= 0; i--) {
          ...
          if (drawing instanceof Polygon && drawing.isComplete) {
            if (drawing.isPointInside(mousePos.x, mousePos.y)) {
              ...
              const fillColor = fillColorPicker ? fillColorPicker.value : '#000000';
      ...
      case 'polygon':
        if (!this.currentPolygon || this.currentPolygon.isComplete) {
          // Créer un nouveau polygone si on n'en a pas ou si le précédent est terminé
        ...
        if (this.currentPolygon.points.length >= 3 && 
          ...
          // Important : mettre currentPolygon à null pour pouvoir en créer un nouveau
          this.currentPolygon = null;
        } else {
  ...
  handleEraserTool(mousePos) {
    ...
    // Vérifier d'abord si on doit supprimer un texte
    if (this.textTool) {
      const textIndex = this.textTool.isPointNear(mousePos.x, mousePos.y, radius + 10);
      if (textIndex !== -1) {
        this.textTool.removeText(textIndex);
        this.redraw();
        return;
  ...
  // Gestionnaires de souris pour le drag and drop des textes
  handleMouseDown(event) {
    if (this.activeTool === 'text' && this.textTool) {
      this.textTool.handleMouseDown(event);
      // Empêcher la propagation seulement si un texte est en cours de drag
      if (this.textTool.isDragging) {
  ...
  handleMouseMove(event) {
    if (this.activeTool === 'text' && this.textTool) {
      this.textTool.handleMouseMove(event);
    }
  }

  handleMouseUp(event) {
    if (this.activeTool === 'text' && this.textTool) {
      const wasDragging = this.textTool.isDragging;
      this.textTool.handleMouseUp(event);
      // Empêcher la propagation seulement si un texte était en cours de drag
      if (wasDragging) {
        event.stopPropagation();
      }
    }
