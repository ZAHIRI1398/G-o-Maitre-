...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Gestionnaire d'événements pour le menu déroulant des outils géométriques
    const geometricTools = document.getElementById('geometric-tools');
    if (geometricTools) {
        geometricTools.addEventListener('change', (e) => {
    ...
    // Gestionnaire d'événements pour le menu déroulant des actions
    const actionTools = document.getElementById('action-tools');
    
    if (actionTools) {
        actionTools.addEventListener('change', (e) => {
            ...
            switch(action) {
                ...
                case 'text':
                    ...
                    break;
                case 'read':
                    // Déclencher le clic sur le bouton de lecture
                    ...
                    break;
                case 'stop-read':
                    // Déclencher le clic sur le bouton stop
            ...
            // Réinitialiser le select après l'action
            e.target.value = '';
        });
    }

    // Gestionnaire d'événements pour les outils de dessin (anciens boutons - pour compatibilité)
                ...
                try {
                    ...
                    fileReader.onload = async function() {
                        ...
                        for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
                            ...
                            // IMPORTANT : Retirer temporairement les écouteurs pendant le redimensionnement
                            canvasPage.canvas.removeEventListener('click', canvasPage.handleClick);
                            canvasPage.canvas.removeEventListener('mousedown', canvasPage.handleMouseDown);
                            canvasPage.canvas.removeEventListener('mousemove', canvasPage.handleMouseMove);
                            canvasPage.canvas.removeEventListener('mouseup', canvasPage.handleMouseUp);
                            
                            // Ajuster la taille du canvas
                            canvasPage.canvas.width = viewport.width;
                            canvasPage.canvas.height = viewport.height;
                            
                            // Redimensionner le pdfCanvas et recréer son contexte
                            canvasPage.pdfCanvas.width = viewport.width;
                            canvasPage.pdfCanvas.height = viewport.height;
                            canvasPage.pdfContext = canvasPage.pdfCanvas.getContext('2d');
                            
                            // Recréer aussi le contexte du canvas principal
                            canvasPage.ctx = canvasPage.canvas.getContext('2d');
                            
                            // Réattacher les écouteurs
                            canvasPage.canvas.addEventListener('click', canvasPage.handleClick);
                            canvasPage.canvas.addEventListener('mousedown', canvasPage.handleMouseDown);
                            canvasPage.canvas.addEventListener('mousemove', canvasPage.handleMouseMove);
                            canvasPage.canvas.addEventListener('mouseup', canvasPage.handleMouseUp);
                            ...
                            try {
                                ...
                                console.log(`Page ${pageNum} rendue dans pdfContext`);
                                
                                // Redessiner tout (copier le pdfCanvas vers le canvas principal)
                                canvasPage.redraw();
                                console.log(`Page ${pageNum} affichée avec succès`);
                            } catch (renderError) {
